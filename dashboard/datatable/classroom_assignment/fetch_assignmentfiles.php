...
`crs`.`status_ID`,
(
    SELECT `craa`.`attachment_Name` FROM `class_room_assignment` `cra` 
    LEFT JOIN `class_room_assignment_attachment` `craa` ON `craa`.`ca_ID` = `cra`.`ca_ID`
    WHERE `cra`.`ca_ID` = ".$_REQUEST['ca_ID']." AND `cra`.`section_ID`  = ".$_REQUEST['section_ID']." AND `craa`.`user_ID` = `rsd`.`user_ID`
) `proj_SubFileName`,
(
    SELECT `craa`.`attachment_MIME` FROM `class_room_assignment` `cra` 
    LEFT JOIN `class_room_assignment_attachment` `craa` ON `craa`.`ca_ID` = `cra`.`ca_ID`
    WHERE `cra`.`ca_ID` = ".$_REQUEST['ca_ID']." AND  `cra`.`section_ID`  = ".$_REQUEST['section_ID']." AND `craa`.`user_ID` = `rsd`.`user_ID` 
) `proj_SubFileMime`,
(
    SELECT `craa`.`attachment_Data` FROM `class_room_assignment` `cra` 
    LEFT JOIN `class_room_assignment_attachment` `craa` ON `craa`.`ca_ID` = `cra`.`ca_ID`
    WHERE `cra`.`ca_ID` = ".$_REQUEST['ca_ID']." AND  `cra`.`section_ID`  = ".$_REQUEST['section_ID']." AND `craa`.`user_ID` = `rsd`.`user_ID` 
) `proj_SubFileData`,
(
    SELECT `craa`.`attachment_ID` FROM `class_room_assignment` `cra` 
    LEFT JOIN `class_room_assignment_attachment` `craa` ON `craa`.`ca_ID` = `cra`.`ca_ID`
    WHERE `cra`.`ca_ID` = ".$_REQUEST['ca_ID']." AND  `cra`.`section_ID`  = ".$_REQUEST['section_ID']." AND `craa`.`user_ID` = `rsd`.`user_ID` 
) `proj_SubFileID`
";
...
if (isset($_REQUEST['classroom_ID']) ||  isset($_REQUEST['section_ID']) ||  isset($_REQUEST['ca_ID'])) {
