...
if(isset($_REQUEST["test_ID"])){
 ...
 if ($stmt4->rowCount() > 0){
    ...
    $stmt5 = $auth_user->runQuery("INSERT INTO `class_room_test_attemp` (`atmp_ID`, `user_ID`, `test_ID`, `count`) 
      VALUES (NULL, ".$_SESSION['user_ID'].", ".$test_ID.", '3');");
    $stmt5->execute();
    $stmt6 = $auth_user->runQuery("SELECT * FROM `class_room_test_attemp` WHERE user_ID = ".$_SESSION['user_ID']." AND test_ID = ".$test_ID."");
    $stmt6->execute();
  ...
  $stmtz9 = $auth_user->runQuery("SELECT * FROM `class_room_test_questions` WHERE test_ID =  ".$test_ID."");
  $stmtz9->execute();
  ...
  <div class="row">
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2"  style="font-size: 16px;"><?php echo $test_Name?></h1><div class="btn btn-info btn-sm float-right " id="retake">RETAKE(<?php echo $retake_count?>)</div>
   ...
   $(document).on('submit', '#test_form', function(event) {
          ...
          $.ajax({
             ...
             success:function(data)
             {
              ...
              function(){
                window.location.assign('result?score_ID='+data.score_ID+'&classroom_ID='+<?php echo $classroom_ID?>);
                 alertify.success('Ok') 
               }).setHeader('Answer');
