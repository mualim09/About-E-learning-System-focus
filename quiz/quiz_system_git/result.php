            ...
            if($total_questions>0){
	         ...
	         //calculating %age
	            for($i=1 ; $i <= $total_questions ; $i++){
	                ...
	                $check_sql = mysqli_query($conn,"SELECT correct FROM answers 
	                                            WHERE id='$php_id'") or die(mysql_error());
	                $q_answer = mysqli_fetch_array($check_sql);
	                $marks += $q_answer[0];
	            }
	            $percent = ($marks/$total_questions)*100;

	         //getting total time taken by the user to complete the quiz
	            $get_time_query = mysqli_query($conn,"SELECT now() - date_time FROM quiz_takers 
	                                            WHERE username = '$roll_no' ") or die(mysqli_error());
	            $get_time = mysqli_fetch_array($get_time_query);
	            $time_taken = $get_time[0];

	            $check_time_query = mysqli_query($conn,"SELECT duration FROM quiz_takers 
	                                            WHERE username = '$roll_no' 
	                                            AND quiz_id = '$quiz_ID' ") or die(mysqli_error());
	            $check_time = mysqli_fetch_array($check_time_query);
	            $duration = $check_time[0];

	            if($duration==0){
		         //updating the %age and time taken by the user in the DB
	            	mysqli_query($conn,"UPDATE quiz_takers 
