<?php
    ...
    */
	...
	$selecting_quiz = mysqli_query($conn,"SELECT quiz_id, display_questions, time_allotted, quiz_name
 ...
 //checking if all 3 values are there
	if(isset($_POST['rollno']) && $_POST['rollno'] != "")
	{
	 ...
	 //checking if user has already taken this quiz
		$userCheck = mysqli_query($conn," SELECT id FROM quiz_takers 
										WHERE username = '$roll_no' 
										AND quiz_id='$final_quiz_ID' ")or die(mysqli_error());
	 ...
	 //else inserting few columns into the table
		mysqli_query($conn,"INSERT INTO quiz_takers (username, percentage, date_time, quiz_id, duration) 
					 VALUES ('$roll_no', '0', now(), '$final_quiz_ID', '0')")or die(mysqli_error());
		}
	}else{
 ...
 //Getting the questions from DB here
	$m_questions_from_DB = mysqli_query($conn,"SELECT * FROM questions WHERE quiz_id='$final_quiz_ID'
	 ...
	 //looping through the questions and adding them on the page
		while($m_row = mysqli_fetch_array($m_questions_from_DB)){
		 ...
		 //gathering options of the question here
			$m_options_from_DB = mysqli_query($conn,"SELECT * FROM answers 
